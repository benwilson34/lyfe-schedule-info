...
[actionable tasks](./recommendations.md#tasks-should-be-actionable).
...
::: info
Considered feature: make schedule/dates optional. Determine how this would work - do these tasks always show up today
...
but it is recommended to [add a time estimate to as many tasks as you can](./recommendations.md#use-time-estimates). 
...
When in [Calendar View](./views.md#calendar-view), repeating tasks will be [projected](./views.md#projected-repeating-tasks) onto 
future days.

::: info
Keep in mind that repeating tasks will repeat based on when the task is **completed** and **do not stack**. For
example, take the task "Wash the dishes" that repeats every day. If you take 5 days to wash the dishes, there will only 
be one task for it which will be 5 days overdue, rather than 5 tasks for it. Once you complete the task, it will repeat 
